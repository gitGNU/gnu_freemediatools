...
public:    
    ...
    flex_string(const std::basic_string<E, T, A>& str, const A& a = A())
    : Storage(str.c_str(), traits_type::length(str.c_str()), a) 
    {}
    ...
    flex_string& assign(const value_type* s, const size_type n)
    {
        ...
        if (size() >= n)
        {
// patch by Goncalo Nuno M. de Carvalho. NOT PART! of Andrei Alexandrescu's official release.
#if		_MSC_VER >= 1400
			stdext::unchecked_copy(s, s + n, begin());
#else
...
// patch by Goncalo Nuno M. de Carvalho. NOT PART! of Andrei Alexandrescu's official release.
#if		_MSC_VER >= 1400
			stdext::unchecked_copy(s, s2, begin());
#else
			std::copy(s, s2, begin());
#endif
            append(s2, n - size());
...
// patch by Goncalo Nuno M. de Carvalho. NOT PART! of Andrei Alexandrescu's official release.
#if		_MSC_VER >= 1400
			stdext::unchecked_copy(reverse_iterator(tailBegin), reverse_iterator(i), 
				reverse_iterator(tailBegin + n2));
			stdext::unchecked_copy(s1, s2, i);
#else
...
// patch by Goncalo Nuno M. de Carvalho. NOT PART! of Andrei Alexandrescu's official release.
#if		_MSC_VER >= 1400
		stdext::unchecked_copy(begin() + pos + n, end(), begin() + pos);
#else
...
// patch by Goncalo Nuno M. de Carvalho. NOT PART! of Andrei Alexandrescu's official release.
#if		_MSC_VER >= 1400
			stdext::unchecked_copy(s1, s2, i1);
#else
			std::copy(s1, s2, i1);
#endif
			erase(i1 + n2, i2);
