...
namespace
{
	inline float audio_convert_db( float db ) { return std::pow( 10.0, db / 20.0 ); }
...
class mydiscovery :
  public  pl::discovery< ml_query_traits > 
{

public:
  mydiscovery (ml_query_traits & query);
    //    :
    //olib::openmedialib::ml::ml_query_traits::ml_query_traits(query)

  //  {
    /*
olib::openmedialib::ml::ml_query_traits::ml_query_traits(const olib::openpluginlib::wstring&, 
...

{
	  ...
	  fprintf( stderr, "openmedialib il get_plug: res: %s type: %s\n", 
		   opl::to_string(resource  ).c_str(),
		   opl::to_string(type  ).c_str()

		   );

  // TODO : mike mdupont disabled this function
  //	typedef pl::discovery< ml_query_traits > discovery;
	  ...
	  mydiscovery plugins( query ); // LINKER PROBLEM
	  if ( plugins.size( ) == 0 ) return result;
	  mydiscovery::const_iterator i = plugins.begin( );
...
*/
	  ...
	  //return boost::shared_dynamic_cast<openmedialib_plugin>(pplug);
...
ML_DECLSPEC store_type_ptr create_store( const pl::wstring &resource, frame_type_ptr frame )
{
	...
	if ( plug == 0 )
	    std::wcerr << "create_store: failed to find a plugin for: " << resource << "\n";
...
ML_DECLSPEC audio_type_ptr audio_reverse( audio_type_ptr audio )
{
	if ( audio )
	{
		...
		short *out = in + channels * samples - 1;
...
mylib.a(libopenmedialib_ml_la-utilities.o): In function `olib::openmedialib::ml::get_plug(std::basic_string<wchar_t, std::char_traits<wchar_t>, std::allocator<wchar_t> > const&, std::basic_string<wchar_t, std::char_traits<wchar_t>, std::allocator<wchar_t> >)':
/mnt/sda10/mdupont/experiments/video/js/openlibraries/trunk/src/openmedialib/ml/utilities.cpp:451: undefined reference to `olib::openpluginlib::discovery<olib::openmedialib::ml::ml_query_traits>::discovery(olib::openmedialib::ml::ml_query_traits const&)'
/mnt/sda10/mdupont/experiments/video/js/openlibraries/trunk/src/openmedialib/ml/utilities.cpp:452: undefined reference to `olib::openpluginlib::discovery<olib::openmedialib::ml::ml_query_traits>::size() const'
/mnt/sda10/mdupont/experiments/video/js/openlibraries/trunk/src/openmedialib/ml/utilities.cpp:453: undefined reference to `olib::openpluginlib::discovery<olib::openmedialib::ml::ml_query_traits>::begin() const'
