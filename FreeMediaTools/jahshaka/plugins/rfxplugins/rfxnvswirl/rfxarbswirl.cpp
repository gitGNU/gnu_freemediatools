...
{
    QString sourceName = tempBasePath+"plugins/rfxplugins/rfxnvswirl/swirl_frag_arb_gpu.fp";
...
void    MyPlugin::processGpuArbFx(void)
{
    ...
    swirl_radius = ((slider[4] * 3.0) + (float_width / 5)) / float_width;
    twist_effect  = (slider[5] / 10.0) + 3.0;
    ...
    {
        fragment_program = loadshaderfile(JahBasePath+"rfxnvswirl/swirl_frag_arb_gpu.fp");
    ...
    {
        find_shader_program_error(fragment_program, "swirl_frag_arb_gpu.fp");
    }
    ...
    glProgramLocalParameter4fARB(GL_FRAGMENT_PROGRAM_ARB, 2, swirl_radius, 0.0f, 0.0f, 0.0f);
    glProgramLocalParameter4fARB(GL_FRAGMENT_PROGRAM_ARB, 3, twist_effect, 0.0f, 0.0f, 0.0f);
