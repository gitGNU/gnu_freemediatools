...
void    MyPlugin::initializePlugin(void)
{
    ...
    pluginName    = "Turbulence";       //name of plugin
    pluginClass   = JAH_GPU_PLUGIN;     //class of plugin ie CPU, GPU, MeshFX

    QString guid = "e6591a86-e0b8-11d9-947b-00e08161165f";
    setGuid(guid);
...
void    MyPlugin::processGpuFx(void)
{
    ...
    {
        vertex_program = loadshaderfile(JahBasePath+"rfxnvturbulence/turbulence_vert_gpu.vp");
    ...
    {
        find_shader_program_error(vertex_program, "turbulence_vert_gpu.vp");
    }
...
//     glProgramParameter4fNV(GL_VERTEX_PROGRAM_NV, 74, 100.0, 0, 0, 0);
    glProgramParameter4fNV(GL_VERTEX_PROGRAM_NV, 78, 2.0f, 1.0f, 10000.0f, 3.0f);
    glProgramParameter4fNV(GL_VERTEX_PROGRAM_NV, 79, 1.0f, 0.5f, 0.0f, 0.0f);
    glProgramParameter4fNV(GL_VERTEX_PROGRAM_NV, 80, 0.03125f, 32.0f, 0.0f, 1.0f);
    ...
    {
        fragment_program = loadshaderfile(JahBasePath+"rfxnvturbulence/turbulence_frag_gpu.fp");
    ...
    if (glGetError() == GL_INVALID_OPERATION)
    {
        find_shader_program_error(fragment_program, "turbulence_frag_gpu.fp");
    }
...
//         if ((speed >= 10) || ((count % (10 - speed)) == 0))
        {
//             noise_translate.x = (float)(-100 + (count % 200)) / speed.x;
//             noise_translate.y = (float)(-100 + (count % 200)) / speed.y;
//             noise_translate.z = (float)((count % 200)) / 300.0;
    ...
    {
        fragment_program = loadshaderfile(JahBasePath+"rfxnvturbulence/turbulence_combiner_frag_gpu.fp");
