!!ARBfp1.0
# ARB_fragment_program generated by NVIDIA Cg compiler
# cgc version 1.2.1001, build date Mar 17 2004  10:32:28
# command line args: -profile arbfp1
#vendor NVIDIA Corporation
#version 1.0.02
#profile arbfp1
#program main
#semantic main.horizontal : C0
#semantic main.vertical : C1
#semantic main.stretch : C2
#semantic main.texture : TEXUNIT0
#var float horizontal : C0 : c[0] : 1 : 1
#var float vertical : C1 : c[1] : 2 : 1
#var float stretch : C2 : c[2] : 3 : 1
#var sampler2D texture : TEXUNIT0 : texunit 0 : 4 : 1
#var float2 position : $vin.TEXCOORD0 : TEX0 : 0 : 1
#var half4 main : $vout.COLOR : COL : -1 : 1
PARAM u0 = program.local[0];
PARAM u1 = program.local[1];
PARAM u2 = program.local[2];
PARAM c0 = {1, 0, 14, 13};
PARAM c1 = {12, 11, 10, 9};
PARAM c2 = {8, 7, 6, 5};
PARAM c3 = {4, 3, 2, 0};
TEMP R0;
TEMP R1;
TEMP R2;
TEMP R3;
TEMP R4;
TEMP R5;
TEMP R6;
TEMP H0;
TEMP H1;
TEMP H2;
TEMP H3;
TEMP H4;
TEMP H5;
TEMP H6;
TEX R0, fragment.texcoord[0], texture[0], 2D;
MOV R1.x, u0.x;
ADD R1.x, c0.y, -R1.x;
CMP H0.x, R1.x, c0.x, c0.y;
MOV R1.x, u1.x;
ADD R1.x, c0.y, -R1.x;
CMP H0.y, R1.x, c0.x, c0.y;
ADD H0.y, H0.x, H0.y;
MIN H0.y, H0.y, c0.x;
MOV R1.x, u0.x;
ADD R1.x, c0.y, -R1.x;
CMP H0.x, R1.x, c0.x, c0.y;
MUL H0.x, H0.y, H0.x;
ADD R1.x, fragment.texcoord[0].x, u2.x;
MOV R2.y, fragment.texcoord[0];
CMP R2.x, -H0.x, R1, fragment.texcoord[0];
MOV R1.x, u1.x;
ADD R1.x, c0.y, -R1.x;
CMP H0.z, R1.x, c0.x, c0.y;
MUL H0.z, H0.y, H0.z;
TEX R1, R2, texture[0], 2D;
ADD H1, R0, R1;
CMP H1, -H0.x, H1, R0;
ADD R0.x, fragment.texcoord[0].x, -u2.x;
MOV R1.y, fragment.texcoord[0];
CMP R1.x, -H0.x, R0, fragment.texcoord[0];
MOV R0.x, u0.x;
ADD R0.x, c0.x, -R0.x;
CMP H0.y, R0.x, c0.x, c0.y;
MOV R0.x, u1.x;
ADD R0.x, c0.x, -R0.x;
CMP H0.w, R0.x, c0.x, c0.y;
ADD H0.w, H0.y, H0.w;
MIN H0.w, H0.w, c0.x;
TEX R0, R1, texture[0], 2D;
ADD H2, H1, R0;
CMP H1, -H0.x, H2, H1;
CMP R0.x, -H0.x, c0.x, c0.y;
ADD R3.x, fragment.texcoord[0].y, u2.x;
MOV R4.x, fragment.texcoord[0];
CMP R4.y, -H0.z, R3.xxzw, fragment.texcoord[0];
MOV R0.y, u0.x;
ADD R0.y, c0.x, -R0.y;
CMP H0.x, R0.y, c0.x, c0.y;
MUL H0.x, H0.w, H0.x;
TEX R3, R4, texture[0], 2D;
ADD H2, H1, R3;
CMP H1, -H0.z, H2, H1;
ADD R3.x, fragment.texcoord[0].y, -u2.x;
MOV R5.x, fragment.texcoord[0];
CMP R5.y, -H0.z, R3.xxzw, fragment.texcoord[0];
TEX R3, R5, texture[0], 2D;
ADD H2, H1, R3;
CMP H1, -H0.z, H2, H1;
MOV R3.y, R2;
ADD R6.x, R2.x, u2.x;
CMP R3.x, -H0.x, R6, R2;
MOV R0.y, u1.x;
ADD R0.y, c0.x, -R0.y;
CMP H0.y, R0.y, c0.x, c0.y;
MUL H0.y, H0.w, H0.y;
TEX R2, R3, texture[0], 2D;
ADD H2, H1, R2;
CMP H1, -H0.x, H2, H1;
MOV R2.y, R1;
ADD R6.x, R1.x, -u2.x;
CMP R2.x, -H0.x, R6, R1;
MOV R0.y, u0.x;
ADD R0.y, c3.z, -R0.y;
CMP H0.w, R0.y, c0.x, c0.y;
MOV R0.y, u1.x;
ADD R0.y, c3.z, -R0.y;
CMP H2.x, R0.y, c0.x, c0.y;
ADD H2.x, H0.w, H2.x;
MIN H2.x, H2.x, c0.x;
TEX R1, R2, texture[0], 2D;
ADD H3, H1, R1;
CMP H1, -H0.x, H3, H1;
MOV R1.x, R4;
ADD R6.x, R4.y, u2.x;
CMP R1.y, -H0.y, R6.xxzw, R4;
MOV R0.y, u0.x;
ADD R0.y, c3.z, -R0.y;
CMP H0.w, R0.y, c0.x, c0.y;
MUL H0.w, H2.x, H0.w;
TEX R4, R1, texture[0], 2D;
ADD H3, H1, R4;
CMP H1, -H0.y, H3, H1;
MOV R4.x, R5;
ADD R6.x, R5.y, -u2.x;
CMP R4.y, -H0.y, R6.xxzw, R5;
TEX R5, R4, texture[0], 2D;
ADD H3, H1, R5;
CMP H1, -H0.y, H3, H1;
MOV R5.y, R3;
ADD R6.x, R3.x, u2.x;
CMP R5.x, -H0.w, R6, R3;
MOV R0.y, u1.x;
ADD R0.y, c3.z, -R0.y;
CMP H2.y, R0.y, c0.x, c0.y;
MUL H2.y, H2.x, H2.y;
TEX R3, R5, texture[0], 2D;
ADD H3, H1, R3;
CMP H1, -H0.w, H3, H1;
MOV R3.y, R2;
ADD R6.x, R2.x, -u2.x;
CMP R3.x, -H0.w, R6, R2;
MOV R0.y, u0.x;
ADD R0.y, c3.y, -R0.y;
CMP H2.x, R0.y, c0.x, c0.y;
MOV R0.y, u1.x;
ADD R0.y, c3.y, -R0.y;
CMP H2.z, R0.y, c0.x, c0.y;
ADD H2.z, H2.x, H2.z;
MIN H2.z, H2.z, c0.x;
TEX R2, R3, texture[0], 2D;
ADD H3, H1, R2;
CMP H1, -H0.w, H3, H1;
MOV R2.x, R1;
ADD R6.x, R1.y, u2.x;
CMP R2.y, -H2.y, R6.xxzw, R1;
MOV R0.y, u0.x;
ADD R0.y, c3.y, -R0.y;
CMP H2.x, R0.y, c0.x, c0.y;
MUL H2.x, H2.z, H2.x;
TEX R1, R2, texture[0], 2D;
ADD H3, H1, R1;
CMP H1, -H2.y, H3, H1;
MOV R1.x, R4;
ADD R6.x, R4.y, -u2.x;
CMP R1.y, -H2.y, R6.xxzw, R4;
TEX R4, R1, texture[0], 2D;
ADD H3, H1, R4;
CMP H1, -H2.y, H3, H1;
MOV R4.y, R5;
ADD R6.x, R5.x, u2.x;
CMP R4.x, -H2.x, R6, R5;
MOV R0.y, u1.x;
ADD R0.y, c3.y, -R0.y;
CMP H2.w, R0.y, c0.x, c0.y;
MUL H2.w, H2.z, H2.w;
TEX R5, R4, texture[0], 2D;
ADD H3, H1, R5;
CMP H1, -H2.x, H3, H1;
MOV R5.y, R3;
ADD R6.x, R3.x, -u2.x;
CMP R5.x, -H2.x, R6, R3;
MOV R0.y, u0.x;
ADD R0.y, c3.x, -R0.y;
CMP H2.z, R0.y, c0.x, c0.y;
MOV R0.y, u1.x;
ADD R0.y, c3.x, -R0.y;
CMP H3.x, R0.y, c0.x, c0.y;
ADD H3.x, H2.z, H3.x;
MIN H3.x, H3.x, c0.x;
TEX R3, R5, texture[0], 2D;
ADD H4, H1, R3;
CMP H1, -H2.x, H4, H1;
MOV R3.x, R2;
ADD R6.x, R2.y, u2.x;
CMP R3.y, -H2.w, R6.xxzw, R2;
MOV R0.y, u0.x;
ADD R0.y, c3.x, -R0.y;
CMP H2.z, R0.y, c0.x, c0.y;
MUL H2.z, H3.x, H2.z;
TEX R2, R3, texture[0], 2D;
ADD H4, H1, R2;
CMP H1, -H2.w, H4, H1;
MOV R2.x, R1;
ADD R6.x, R1.y, -u2.x;
CMP R2.y, -H2.w, R6.xxzw, R1;
TEX R1, R2, texture[0], 2D;
ADD H4, H1, R1;
CMP H1, -H2.w, H4, H1;
MOV R1.y, R4;
ADD R6.x, R4.x, u2.x;
CMP R1.x, -H2.z, R6, R4;
MOV R0.y, u1.x;
ADD R0.y, c3.x, -R0.y;
CMP H3.y, R0.y, c0.x, c0.y;
MUL H3.y, H3.x, H3.y;
TEX R4, R1, texture[0], 2D;
ADD H4, H1, R4;
CMP H1, -H2.z, H4, H1;
MOV R4.y, R5;
ADD R6.x, R5.x, -u2.x;
CMP R4.x, -H2.z, R6, R5;
MOV R0.y, u0.x;
ADD R0.y, c2.w, -R0.y;
CMP H3.x, R0.y, c0.x, c0.y;
MOV R0.y, u1.x;
ADD R0.y, c2.w, -R0.y;
CMP H3.z, R0.y, c0.x, c0.y;
ADD H3.z, H3.x, H3.z;
MIN H3.z, H3.z, c0.x;
TEX R5, R4, texture[0], 2D;
ADD H4, H1, R5;
CMP H1, -H2.z, H4, H1;
MOV R5.x, R3;
ADD R6.x, R3.y, u2.x;
CMP R5.y, -H3.y, R6.xxzw, R3;
MOV R0.y, u0.x;
ADD R0.y, c2.w, -R0.y;
CMP H3.x, R0.y, c0.x, c0.y;
MUL H3.x, H3.z, H3.x;
TEX R3, R5, texture[0], 2D;
ADD H4, H1, R3;
CMP H1, -H3.y, H4, H1;
MOV R3.x, R2;
ADD R6.x, R2.y, -u2.x;
CMP R3.y, -H3.y, R6.xxzw, R2;
TEX R2, R3, texture[0], 2D;
ADD H4, H1, R2;
CMP H1, -H3.y, H4, H1;
MOV R2.y, R1;
ADD R6.x, R1.x, u2.x;
CMP R2.x, -H3.x, R6, R1;
MOV R0.y, u1.x;
ADD R0.y, c2.w, -R0.y;
CMP H3.w, R0.y, c0.x, c0.y;
MUL H3.w, H3.z, H3.w;
TEX R1, R2, texture[0], 2D;
ADD H4, H1, R1;
CMP H1, -H3.x, H4, H1;
MOV R1.y, R4;
ADD R6.x, R4.x, -u2.x;
CMP R1.x, -H3.x, R6, R4;
MOV R0.y, u0.x;
ADD R0.y, c2.z, -R0.y;
CMP H3.z, R0.y, c0.x, c0.y;
MOV R0.y, u1.x;
ADD R0.y, c2.z, -R0.y;
CMP H4.x, R0.y, c0.x, c0.y;
ADD H4.x, H3.z, H4.x;
MIN H4.x, H4.x, c0.x;
TEX R4, R1, texture[0], 2D;
ADD H5, H1, R4;
CMP H1, -H3.x, H5, H1;
MOV R4.x, R5;
ADD R6.x, R5.y, u2.x;
CMP R4.y, -H3.w, R6.xxzw, R5;
MOV R0.y, u0.x;
ADD R0.y, c2.z, -R0.y;
CMP H3.z, R0.y, c0.x, c0.y;
MUL H3.z, H4.x, H3.z;
TEX R5, R4, texture[0], 2D;
ADD H5, H1, R5;
CMP H1, -H3.w, H5, H1;
MOV R5.x, R3;
ADD R6.x, R3.y, -u2.x;
CMP R5.y, -H3.w, R6.xxzw, R3;
TEX R3, R5, texture[0], 2D;
ADD H5, H1, R3;
CMP H1, -H3.w, H5, H1;
MOV R3.y, R2;
ADD R6.x, R2.x, u2.x;
CMP R3.x, -H3.z, R6, R2;
MOV R0.y, u1.x;
ADD R0.y, c2.z, -R0.y;
CMP H4.y, R0.y, c0.x, c0.y;
MUL H4.y, H4.x, H4.y;
TEX R2, R3, texture[0], 2D;
ADD H5, H1, R2;
CMP H1, -H3.z, H5, H1;
MOV R2.y, R1;
ADD R6.x, R1.x, -u2.x;
CMP R2.x, -H3.z, R6, R1;
MOV R0.y, u0.x;
ADD R0.y, c2.y, -R0.y;
CMP H4.x, R0.y, c0.x, c0.y;
MOV R0.y, u1.x;
ADD R0.y, c2.y, -R0.y;
CMP H4.z, R0.y, c0.x, c0.y;
ADD H4.z, H4.x, H4.z;
MIN H4.z, H4.z, c0.x;
TEX R1, R2, texture[0], 2D;
ADD H5, H1, R1;
CMP H1, -H3.z, H5, H1;
MOV R1.x, R4;
ADD R6.x, R4.y, u2.x;
CMP R1.y, -H4.y, R6.xxzw, R4;
MOV R0.y, u0.x;
ADD R0.y, c2.y, -R0.y;
CMP H4.x, R0.y, c0.x, c0.y;
MUL H4.x, H4.z, H4.x;
TEX R4, R1, texture[0], 2D;
ADD H5, H1, R4;
CMP H1, -H4.y, H5, H1;
MOV R4.x, R5;
ADD R6.x, R5.y, -u2.x;
CMP R4.y, -H4.y, R6.xxzw, R5;
TEX R5, R4, texture[0], 2D;
ADD H5, H1, R5;
CMP H1, -H4.y, H5, H1;
MOV R5.y, R3;
ADD R6.x, R3.x, u2.x;
CMP R5.x, -H4.x, R6, R3;
MOV R0.y, u1.x;
ADD R0.y, c2.y, -R0.y;
CMP H4.w, R0.y, c0.x, c0.y;
MUL H4.w, H4.z, H4.w;
TEX R3, R5, texture[0], 2D;
ADD H5, H1, R3;
CMP H1, -H4.x, H5, H1;
MOV R3.y, R2;
ADD R6.x, R2.x, -u2.x;
CMP R3.x, -H4.x, R6, R2;
MOV R0.y, u0.x;
ADD R0.y, c2.x, -R0.y;
CMP H4.z, R0.y, c0.x, c0.y;
MOV R0.y, u1.x;
ADD R0.y, c2.x, -R0.y;
CMP H5.x, R0.y, c0.x, c0.y;
ADD H5.x, H4.z, H5.x;
MIN H5.x, H5.x, c0.x;
TEX R2, R3, texture[0], 2D;
ADD H6, H1, R2;
CMP H1, -H4.x, H6, H1;
MOV R2.x, R1;
ADD R6.x, R1.y, u2.x;
CMP R2.y, -H4.w, R6.xxzw, R1;
MOV R0.y, u0.x;
ADD R0.y, c2.x, -R0.y;
CMP H4.z, R0.y, c0.x, c0.y;
MUL H4.z, H5.x, H4.z;
TEX R1, R2, texture[0], 2D;
ADD H6, H1, R1;
CMP H1, -H4.w, H6, H1;
MOV R1.x, R4;
ADD R6.x, R4.y, -u2.x;
CMP R1.y, -H4.w, R6.xxzw, R4;
ADD R0.y, R0.x, c0.x;
CMP R0.x, -H0.z, R0.y, R0.x;
TEX R4, R1, texture[0], 2D;
ADD H6, H1, R4;
CMP H1, -H4.w, H6, H1;
MOV R4.y, R5;
ADD R6.x, R5.x, u2.x;
CMP R4.x, -H4.z, R6, R5;
ADD R0.y, R0.x, c0.x;
CMP R0.x, -H0.x, R0.y, R0.x;
MOV R0.y, u1.x;
ADD R0.y, c2.x, -R0.y;
CMP H0.x, R0.y, c0.x, c0.y;
MUL H0.x, H5.x, H0.x;
TEX R5, R4, texture[0], 2D;
ADD H5, H1, R5;
CMP H1, -H4.z, H5, H1;
MOV R5.y, R3;
ADD R6.x, R3.x, -u2.x;
CMP R5.x, -H4.z, R6, R3;
ADD R0.y, R0.x, c0.x;
CMP R0.x, -H0.y, R0.y, R0.x;
MOV R0.y, u0.x;
ADD R0.y, c1.w, -R0.y;
CMP H0.y, R0.y, c0.x, c0.y;
MOV R0.y, u1.x;
ADD R0.y, c1.w, -R0.y;
CMP H0.z, R0.y, c0.x, c0.y;
ADD H0.z, H0.y, H0.z;
MIN H0.z, H0.z, c0.x;
TEX R3, R5, texture[0], 2D;
ADD H5, H1, R3;
CMP H1, -H4.z, H5, H1;
MOV R3.x, R2;
ADD R6.x, R2.y, u2.x;
CMP R3.y, -H0.x, R6.xxzw, R2;
ADD R0.y, R0.x, c0.x;
CMP R0.x, -H0.w, R0.y, R0.x;
MOV R0.y, u0.x;
ADD R0.y, c1.w, -R0.y;
CMP H0.y, R0.y, c0.x, c0.y;
MUL H0.y, H0.z, H0.y;
TEX R2, R3, texture[0], 2D;
ADD H5, H1, R2;
CMP H1, -H0.x, H5, H1;
MOV R2.x, R1;
ADD R6.x, R1.y, -u2.x;
CMP R2.y, -H0.x, R6.xxzw, R1;
ADD R0.y, R0.x, c0.x;
CMP R0.x, -H2.y, R0.y, R0.x;
TEX R1, R2, texture[0], 2D;
ADD H5, H1, R1;
CMP H1, -H0.x, H5, H1;
MOV R1.y, R4;
ADD R6.x, R4.x, u2.x;
CMP R1.x, -H0.y, R6, R4;
ADD R0.y, R0.x, c0.x;
CMP R0.x, -H2.x, R0.y, R0.x;
MOV R0.y, u1.x;
ADD R0.y, c1.w, -R0.y;
CMP H0.w, R0.y, c0.x, c0.y;
MUL H0.w, H0.z, H0.w;
TEX R4, R1, texture[0], 2D;
ADD H5, H1, R4;
CMP H1, -H0.y, H5, H1;
MOV R4.y, R5;
ADD R6.x, R5.x, -u2.x;
CMP R4.x, -H0.y, R6, R5;
ADD R0.y, R0.x, c0.x;
CMP R0.x, -H2.w, R0.y, R0.x;
MOV R0.y, u0.x;
ADD R0.y, c1.z, -R0.y;
CMP H0.z, R0.y, c0.x, c0.y;
MOV R0.y, u1.x;
ADD R0.y, c1.z, -R0.y;
CMP H2.x, R0.y, c0.x, c0.y;
ADD H2.x, H0.z, H2.x;
MIN H2.x, H2.x, c0.x;
TEX R5, R4, texture[0], 2D;
ADD H5, H1, R5;
CMP H1, -H0.y, H5, H1;
MOV R5.x, R3;
ADD R6.x, R3.y, u2.x;
CMP R5.y, -H0.w, R6.xxzw, R3;
ADD R0.y, R0.x, c0.x;
CMP R0.x, -H2.z, R0.y, R0.x;
MOV R0.y, u0.x;
ADD R0.y, c1.z, -R0.y;
CMP H0.z, R0.y, c0.x, c0.y;
MUL H0.z, H2.x, H0.z;
TEX R3, R5, texture[0], 2D;
ADD H5, H1, R3;
CMP H1, -H0.w, H5, H1;
MOV R3.x, R2;
ADD R6.x, R2.y, -u2.x;
CMP R3.y, -H0.w, R6.xxzw, R2;
ADD R0.y, R0.x, c0.x;
CMP R0.x, -H3.y, R0.y, R0.x;
TEX R2, R3, texture[0], 2D;
ADD H5, H1, R2;
CMP H1, -H0.w, H5, H1;
MOV R2.y, R1;
ADD R6.x, R1.x, u2.x;
CMP R2.x, -H0.z, R6, R1;
ADD R0.y, R0.x, c0.x;
CMP R0.x, -H3.x, R0.y, R0.x;
MOV R0.y, u1.x;
ADD R0.y, c1.z, -R0.y;
CMP H2.y, R0.y, c0.x, c0.y;
MUL H2.y, H2.x, H2.y;
TEX R1, R2, texture[0], 2D;
ADD H5, H1, R1;
CMP H1, -H0.z, H5, H1;
MOV R1.y, R4;
ADD R6.x, R4.x, -u2.x;
CMP R1.x, -H0.z, R6, R4;
ADD R0.y, R0.x, c0.x;
CMP R0.x, -H3.w, R0.y, R0.x;
MOV R0.y, u0.x;
ADD R0.y, c1.y, -R0.y;
CMP H2.x, R0.y, c0.x, c0.y;
MOV R0.y, u1.x;
ADD R0.y, c1.y, -R0.y;
CMP H2.z, R0.y, c0.x, c0.y;
ADD H2.z, H2.x, H2.z;
MIN H2.z, H2.z, c0.x;
TEX R4, R1, texture[0], 2D;
ADD H5, H1, R4;
CMP H1, -H0.z, H5, H1;
MOV R4.x, R5;
ADD R6.x, R5.y, u2.x;
CMP R4.y, -H2.y, R6.xxzw, R5;
ADD R0.y, R0.x, c0.x;
CMP R0.x, -H3.z, R0.y, R0.x;
MOV R0.y, u0.x;
ADD R0.y, c1.y, -R0.y;
CMP H2.x, R0.y, c0.x, c0.y;
MUL H2.x, H2.z, H2.x;
TEX R5, R4, texture[0], 2D;
ADD H3, H1, R5;
CMP H1, -H2.y, H3, H1;
MOV R5.x, R3;
ADD R6.x, R3.y, -u2.x;
CMP R5.y, -H2.y, R6.xxzw, R3;
ADD R0.y, R0.x, c0.x;
CMP R0.x, -H4.y, R0.y, R0.x;
TEX R3, R5, texture[0], 2D;
ADD H3, H1, R3;
CMP H1, -H2.y, H3, H1;
MOV R3.y, R2;
ADD R6.x, R2.x, u2.x;
CMP R3.x, -H2.x, R6, R2;
ADD R0.y, R0.x, c0.x;
CMP R0.x, -H4.x, R0.y, R0.x;
MOV R0.y, u1.x;
ADD R0.y, c1.y, -R0.y;
CMP H2.w, R0.y, c0.x, c0.y;
MUL H2.w, H2.z, H2.w;
TEX R2, R3, texture[0], 2D;
ADD H3, H1, R2;
CMP H1, -H2.x, H3, H1;
MOV R2.y, R1;
ADD R6.x, R1.x, -u2.x;
CMP R2.x, -H2.x, R6, R1;
ADD R0.y, R0.x, c0.x;
CMP R0.x, -H4.w, R0.y, R0.x;
MOV R0.y, u0.x;
ADD R0.y, c1.x, -R0.y;
CMP H2.z, R0.y, c0.x, c0.y;
MOV R0.y, u1.x;
ADD R0.y, c1.x, -R0.y;
CMP H3.x, R0.y, c0.x, c0.y;
ADD H3.x, H2.z, H3.x;
MIN H3.x, H3.x, c0.x;
TEX R1, R2, texture[0], 2D;
ADD H5, H1, R1;
CMP H1, -H2.x, H5, H1;
MOV R1.x, R4;
ADD R6.x, R4.y, u2.x;
CMP R1.y, -H2.w, R6.xxzw, R4;
ADD R0.y, R0.x, c0.x;
CMP R0.x, -H4.z, R0.y, R0.x;
MOV R0.y, u0.x;
ADD R0.y, c1.x, -R0.y;
CMP H2.z, R0.y, c0.x, c0.y;
MUL H2.z, H3.x, H2.z;
TEX R4, R1, texture[0], 2D;
ADD H4, H1, R4;
CMP H1, -H2.w, H4, H1;
MOV R4.x, R5;
ADD R6.x, R5.y, -u2.x;
CMP R4.y, -H2.w, R6.xxzw, R5;
ADD R0.y, R0.x, c0.x;
CMP R0.x, -H0.x, R0.y, R0.x;
TEX R5, R4, texture[0], 2D;
ADD H4, H1, R5;
CMP H1, -H2.w, H4, H1;
MOV R5.y, R3;
ADD R6.x, R3.x, u2.x;
CMP R5.x, -H2.z, R6, R3;
ADD R0.y, R0.x, c0.x;
CMP R0.x, -H0.y, R0.y, R0.x;
MOV R0.y, u1.x;
ADD R0.y, c1.x, -R0.y;
CMP H0.x, R0.y, c0.x, c0.y;
MUL H0.x, H3.x, H0.x;
TEX R3, R5, texture[0], 2D;
ADD H3, H1, R3;
CMP H1, -H2.z, H3, H1;
MOV R3.y, R2;
ADD R6.x, R2.x, -u2.x;
CMP R3.x, -H2.z, R6, R2;
ADD R0.y, R0.x, c0.x;
CMP R0.x, -H0.w, R0.y, R0.x;
MOV R0.y, u0.x;
ADD R0.y, c0.w, -R0.y;
CMP H0.y, R0.y, c0.x, c0.y;
MOV R0.y, u1.x;
ADD R0.y, c0.w, -R0.y;
CMP H0.w, R0.y, c0.x, c0.y;
ADD H0.w, H0.y, H0.w;
MIN H0.w, H0.w, c0.x;
TEX R2, R3, texture[0], 2D;
ADD H3, H1, R2;
CMP H1, -H2.z, H3, H1;
MOV R2.x, R1;
ADD R6.x, R1.y, u2.x;
CMP R2.y, -H0.x, R6.xxzw, R1;
ADD R0.y, R0.x, c0.x;
CMP R0.x, -H0.z, R0.y, R0.x;
MOV R0.y, u0.x;
ADD R0.y, c0.w, -R0.y;
CMP H0.y, R0.y, c0.x, c0.y;
MUL H0.y, H0.w, H0.y;
TEX R1, R2, texture[0], 2D;
ADD H3, H1, R1;
CMP H1, -H0.x, H3, H1;
MOV R1.x, R4;
ADD R6.x, R4.y, -u2.x;
CMP R1.y, -H0.x, R6.xxzw, R4;
ADD R0.y, R0.x, c0.x;
CMP R0.x, -H2.y, R0.y, R0.x;
TEX R4, R1, texture[0], 2D;
ADD H3, H1, R4;
CMP H1, -H0.x, H3, H1;
MOV R4.y, R5;
ADD R6.x, R5.x, u2.x;
CMP R4.x, -H0.y, R6, R5;
ADD R0.y, R0.x, c0.x;
CMP R0.x, -H2.x, R0.y, R0.x;
MOV R0.y, u1.x;
ADD R0.y, c0.w, -R0.y;
CMP H0.z, R0.y, c0.x, c0.y;
MUL H0.z, H0.w, H0.z;
TEX R5, R4, texture[0], 2D;
ADD H3, H1, R5;
CMP H1, -H0.y, H3, H1;
MOV R5.y, R3;
ADD R6.x, R3.x, -u2.x;
CMP R5.x, -H0.y, R6, R3;
ADD R0.y, R0.x, c0.x;
CMP R0.x, -H2.w, R0.y, R0.x;
TEX R3, R5, texture[0], 2D;
ADD H3, H1, R3;
CMP H1, -H0.y, H3, H1;
MOV R3.x, R2;
ADD R6.x, R2.y, u2.x;
CMP R3.y, -H0.z, R6.xxzw, R2;
ADD R0.y, R0.x, c0.x;
CMP R0.x, -H2.z, R0.y, R0.x;
TEX R2, R3, texture[0], 2D;
ADD H2, H1, R2;
CMP H1, -H0.z, H2, H1;
MOV R2.x, R1;
ADD R6.x, R1.y, -u2.x;
CMP R2.y, -H0.z, R6.xxzw, R1;
MOV R0.y, u0.x;
ADD R0.y, c0.z, -R0.y;
CMP H0.w, R0.y, c0.x, c0.y;
MOV R0.y, u1.x;
ADD R0.y, c0.z, -R0.y;
CMP H2.x, R0.y, c0.x, c0.y;
ADD H2.x, H0.w, H2.x;
MIN H2.x, H2.x, c0.x;
ADD R0.y, R0.x, c0.x;
CMP R0.x, -H0.x, R0.y, R0.x;
TEX R1, R2, texture[0], 2D;
ADD H3, H1, R1;
CMP H1, -H0.z, H3, H1;
MOV R1.y, R4.y;
ADD R0.y, R4.x, u2.x;
MOV R1.x, R0.y;
TEX R1, R1, texture[0], 2D;
ADD H3, H1, R1;
MOV R0.y, u0.x;
ADD R0.y, c0.z, -R0.y;
CMP H0.x, R0.y, c0.x, c0.y;
MUL H0.x, H2.x, H0.x;
CMP H1, -H0.x, H3, H1;
ADD R0.y, R0.x, c0.x;
CMP R0.x, -H0.y, R0.y, R0.x;
MOV R1.y, R5.y;
ADD R0.y, R5.x, -u2.x;
MOV R1.x, R0.y;
TEX R1, R1, texture[0], 2D;
ADD H3, H1, R1;
CMP H1, -H0.x, H3, H1;
ADD R0.y, R0.x, c0.x;
CMP R0.x, -H0.z, R0.y, R0.x;
MOV R1.x, R3.x;
ADD R0.y, R3.y, u2.x;
MOV R1.y, R0.y;
TEX R1, R1, texture[0], 2D;
ADD H3, H1, R1;
MOV R0.y, u1.x;
ADD R0.y, c0.z, -R0.y;
CMP H0.y, R0.y, c0.x, c0.y;
MUL H0.y, H2.x, H0.y;
CMP H1, -H0.y, H3, H1;
ADD R0.y, R0.x, c0.x;
CMP R0.x, -H0.x, R0.y, R0.x;
MOV R1.x, R2.x;
ADD R0.y, R2.y, -u2.x;
MOV R1.y, R0.y;
TEX R1, R1, texture[0], 2D;
ADD H2, H1, R1;
CMP H1, -H0.y, H2, H1;
ADD R0.y, R0.x, c0.x;
CMP R0.x, -H0.y, R0.y, R0.x;
MOV R0.y, c0.x;
MAD R0.y, R0.x, c3.z, R0.y;
RCP R0.y, R0.y;
ADD R0.x, c0.y, -R0.x;
CMP H0.x, R0.x, c0.x, c0.y;
MUL R0, H1, R0.y;
CMP result.color, -H0.x, R0, H1;
END
# 667 instructions, 7 R-regs, 7 H-regs.
# End of program
