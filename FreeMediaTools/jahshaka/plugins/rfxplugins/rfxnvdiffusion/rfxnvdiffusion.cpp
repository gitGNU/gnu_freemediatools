...
void    MyPlugin::initializePlugin(void)
{
    ...
    pluginName    = "Diffusion";        //name of plugin
    pluginClass   = JAH_GPU_PLUGIN;     //class of plugin ie CPU, GPU, MeshFX

    QString guid = "764cf6e0-e0b8-11d9-947b-00e08161165f";
    setGuid(guid);
...
void    MyPlugin::processGpuFx(void)
{
    ...
    gradient = slider[2] / 100.0;
    cosine_factor = -slider[3] / 100.0;

    number_of_layers = 5;

    distortion.x = ((slider[4]) / 100.0) + 0.1;
    distortion.y = ((slider[5]) / 100.0) + 0.1;
    intensity = (slider[6] / 10.0) + 3.0;

    horizontal_mesh_size = (int)((slider[7] * 10.0) + 5.0);
    horizontal_mesh_size = (horizontal_mesh_size > width) ? width : horizontal_mesh_size;

    vertical_mesh_size = ((int)(slider[8] * 10));
    ...
    createTurbulenceTextureGPU(width, height, intensity, horizontal_mesh_size, vertical_mesh_size, number_of_layers);
    ...
    {
        find_shader_program_error(vertex_program, "diffusion_vert_gpu.fp");
    }
    ///////////////////////////////////////////////////
    ...
    fragment_program = loadshaderfile(JahBasePath+"rfxnvdiffusion/diffusion_frag_gpu.fp");
    ...
    glProgramNamedParameter4fNV(fragment_program_handle, strlen("time"),
        (const GLubyte*)"time", (float)count / 100.0, 0.0f, 0.0f, 0.0f);
