...
void    MyPlugin::initializePlugin(void)
{
    ...
    pluginName    = "Flame";            //name of plugin
    pluginClass   = JAH_GPU_PLUGIN;     //class of plugin ie CPU, GPU, MeshFX

    QString guid = "8d7ff89f-e0b8-11d9-947b-00e08161165f";
    setGuid(guid);
...
void    MyPlugin::processGpuFx(void)
{
    ...
    float                                       no_distortion_radius = 0.0;
    ...
    Displacement.y = (slider[2] / 1.0) + 800;
    Displacement.z = (slider[3]);

    intensity = (slider[4] / 10.0) + 15.0;
    transparency = ((100 - slider[5]) / 100.0) - 0.5;
    blend = ((100 - slider[6]) / 100.0) - 0.5;

    horizontal_mesh_size = (int)((slider[7] * 50.0) - 40.0);
    horizontal_mesh_size = (horizontal_mesh_size > width) ? width : horizontal_mesh_size;

    vertical_mesh_size = (int)((slider[8] * 50.0) - 45.0);
    vertical_mesh_size = (vertical_mesh_size > height) ? height : vertical_mesh_size;

    flame_height = (slider[9] / 5.0) + 0.3;
    ...
    {
        vertex_program = loadshaderfile(JahBasePath+"rfxnvflame/flame_vert_gpu.vp");
    ...
    {
        find_shader_program_error(vertex_program, "flame_vert_gpu.vp");
    }
...
//     glProgramParameter4fNV(GL_VERTEX_PROGRAM_NV, 74, 100.0, 0, 0, 0);
    ...
    {
        fragment_program = loadshaderfile(JahBasePath+"rfxnvflame/flame_frag_gpu.fp");
    ...
    if (glGetError() == GL_INVALID_OPERATION)
    {
        find_shader_program_error(fragment_program, "flame_frag_gpu.fp");
    }
    ...
    for (int layer = 0; layer < 4; layer++)
    {
        ...
        noise_translate.x = (-100.0 + noisecount + ((float)(rand() % 100) / 50.0) ) / movement.x;
        noise_translate.y = (-100.0 + noisecount + ((float)(rand() % 100) / 50.0) ) / movement.y;
        noise_translate.z = noisecount / 300.0;
    ...
    ///////////////////////////////////////////////////////////////////////////////////////////
    // now we do it all again using the regular vert engine
    ...
    {
        fragment_program = loadshaderfile(JahBasePath+"rfxnvflame/flame_combiner_frag_gpu.fp");
    ...
    if (glGetError() == GL_INVALID_OPERATION)
    {
        find_shader_program_error(fragment_program, "flame_combiner_frag_gpu.fp");
    }
    ...
    if (TURBULENCE_TEXTURE_SIZE > width)
    {
        scale_factor_x = 1.0 / (float)(TURBULENCE_TEXTURE_SIZE / width);
    }
    else
    {
        scale_factor_x = 1.0;
    ...
    if (TURBULENCE_TEXTURE_SIZE > height)
    {
        scale_factor_y = 1.0 / (float)(TURBULENCE_TEXTURE_SIZE / height);
    }
    else
    {
        scale_factor_y = 1.0;
    ...
    glProgramNamedParameter4fNV(fragment_program_handle, strlen("flame_height"),
        (const GLubyte*)"flame_height", flame_height, 0.0f, 0.0f, 0.0f);

    glProgramNamedParameter4fNV(fragment_program_handle, strlen("count"),
        (const GLubyte*)"count", count, 0.0f, 0.0f, 0.0f);
   ...
   ////////////////////////////////////////////
    ...
    {
        fragment_program = loadshaderfile(JahBasePath+"rfxnvflame/flame_blur_frag_gpu.fp");
    ...
    if (glGetError() == GL_INVALID_OPERATION)
    {
        find_shader_program_error(fragment_program, "flame_blur_frag_gpu.fp");
    }
    ...
    glProgramNamedParameter4fNV(fragment_program_handle, strlen("stretch"),
        (const GLubyte*)"stretch", 44.0 / 10000.0, scale_factor_y, 0.0f, 0.0f);

    glProgramNamedParameter4fNV(fragment_program_handle, strlen("horizontal"),
        (const GLubyte*)"horizontal", 1.0, 0.0f, 0.0f, 0.0f);

    glProgramNamedParameter4fNV(fragment_program_handle, strlen("vertical"),
        (const GLubyte*)"vertical", 3.0, 0.0f, 0.0f, 0.0f);
