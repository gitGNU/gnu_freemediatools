...
GLWorld::chooseNewMeshEffect(LayerListEntry* parent, QWidget* thebutton, int x_offset, int y_offset)
{
    ...
    {
        EffectInfo* effect = m_aMeshEffectsSelectionVector[i];
...
GLWorld::addMeshEffectLayer(int effect_id, JahLayer* parent_jah_layer, const char* effect_name)
{
    ...
    if ( effect_id != -1 && parent_jah_layer->layerAssetStatus )
    {
        ...
        {
            effect_type = m_aMeshEffectsSelectionVector[vector_index]->getType();
            title = QString( (*m_aMeshEffectsSelectionVector[vector_index]->getName(effect_type) ).data() );
        }
        else
        {
            QString thename = effect_name;
            plugin_id = m_pJPlugIn->getPluginId(thename, JAH_MESH_PLUGIN);
...
void GLWorld::doSwirlRt(JahLayer* base_layer)
{
    ...
    for (int i = 0; i <= base_layer->JAHMESH_X_DIMENSION; i++) 
    {
        ...
        {
            //layers[1].getMeshCoord(i, j, xx, yy, m_fZOffset);
            base_layer->getMeshCoord(i, j, xx, yy, m_fZOffset);
            ...
            { 
                ...
                base_layer->setMeshCoord(i, j, new_xx, new_yy, m_fZOffset);                   
            }
            else
            {
                base_layer->setMeshCoord(i, j, xx, yy, m_fZOffset); 
            }
        }
    }
...
void GLWorld::doFlagRt(JahLayer* base_layer) {
    ...
    for (i = 0; i <= base_layer->JAHMESH_X_DIMENSION; i++)
    {
        ...
        {
            base_layer->getMeshCoord(i, j, xx, yy, m_fZOffset);
            zz = m_fZOffset - (magnitude * (float)sin(radians));
    ...
    for (i = 0; i <= base_layer->JAHMESH_X_DIMENSION; i++) 
    {
        for (j = 0; j <= (base_layer->JAHMESH_Y_DIMENSION + 1); j++) 
        {
            ...
            if ((current_radius < outer_limit) && (current_radius > inner_limit))
            {
                ...
                zz = m_fZOffset + ((wave_height * cos(radians)) * 5.0);
            }
            else
            {
                zz = m_fZOffset;
