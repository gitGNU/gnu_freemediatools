...
GLWorld::doColorizeGPU(JahLayer* base_layer) 
{
    ...
    if ( jahstd::glslSupport() && nv_gpusupport ) //nvidia support for the color corrector
    {
        //doNV30ColorizeGPU(base_layer);
        doColorCorrectorGLSL(base_layer);
    }
    else if ( gpusupport ) //nvidia support for the color corrector
    {
        ...
        if (!testarbsupport)
        {
            ...
            QString sourceName = JahBasePath+"source/Jahshaka/JahLibraries/shaders/color_frag_arb_gpu.fp";
...
GLWorld::doNV30ColorizeGPU(JahLayer* base_layer) 
{
    ...
    if (count == 0)
    {
        fragment_program = loadshaderfile(JahBasePath+"source/Jahshaka/JahLibraries/shaders/color_frag_gpu.fp");
    }
...
GLWorld::doArbColorizeGPU(JahLayer* base_layer) 
{
    ...
    if (count == 0)
    {
        fragment_program = loadshaderfile(JahBasePath+"source/Jahshaka/JahLibraries/shaders/color_frag_arb_gpu.fp");
    }
...
const char* 
GLWorld::m_color_correction_fragment_shader =
{
        "   float                                                                   \n"
        "   maxColor( vec3 color)                                                   \n"
        "   {                                                                       \n"
...
GLWorld::doColorCorrectorGLSL(JahLayer* image_layer)
{
    ...
    {
        fragment_shader = new GlslShader(GL_FRAGMENT_SHADER_ARB, m_color_correction_fragment_shader);
    ...
    int render_width = core->getRenderWidth();
    int render_height = core->getRenderHeight();
