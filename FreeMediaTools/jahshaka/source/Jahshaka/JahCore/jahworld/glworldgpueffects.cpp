...
bool GLWorld::sm_bNoiseMatrixInitialized = false;
bool GLWorld::sm_bContrastTextureInitialized = false;
bool GLWorld::sm_bRandomTextureInitialized = false;
int* GLWorld::sm_pPermutationTable = NULL;
float4* GLWorld::sm_pGradientTable = NULL;

GLuint GLWorld::sm_nGradientTexture;
GLuint GLWorld::sm_nPermutationTexture;
GLuint GLWorld::sm_nTurbulenceTexture;
GLuint GLWorld::sm_nRandomTexture;
GLuint GLWorld::sm_nContrastTexture;
...
#define USE_OFFSCREEN_RENDERING         FALSE
    ...
    if (!m_bNoiseMatrixInitialized)
    {
        initVertexNoiseConstants(DISTORTION_TABLE_SIZE, sm_pPermutationTable, m_pGradientTable, GRADIENT_TABLE_SIZE);
        //        initializeMesh(float_width, float_height);
        m_bNoiseMatrixInitialized = true;
    }
    ...
    vertex_program = loadshaderfile(m_qsJahBasePath+"source/Jahshaka/JahLibraries/shaders/earthquake_vert_gpu.vp");
    ...
    glProgramParameters4fvNV(GL_VERTEX_PROGRAM_NV, 8, DISTORTION_TABLE_SIZE * 2, (float*)m_pGradientTable);
    glProgramParameter4fvNV(GL_VERTEX_PROGRAM_NV, 8 + (DISTORTION_TABLE_SIZE * 2), (float*)&m_pGradientTable[0]);
    glProgramParameter4fvNV(GL_VERTEX_PROGRAM_NV, 8 + (DISTORTION_TABLE_SIZE * 2) + 1, (float*)&m_pGradientTable[1]);
    ...
    fragment_program = loadshaderfile(m_qsJahBasePath+"source/Jahshaka/JahLibraries/shaders/earthquake_frag_gpu.fp");
...
void GLWorld::doNV30CharcoalGPU(JahLayer* base_layer) {
    ...
    if (!sm_bRandomTextureInitialized)
    {
        createRandomTexture(512, 512, m_hRandomTexture);
        sm_bRandomTextureInitialized = TRUE;
    }
    
    if (!sm_bContrastTextureInitialized || (exponent != previous_exponent))
    {
        if (sm_bContrastTextureInitialized)
        {
            glDeleteTextures(1, &sm_nContrastTexture);
        }
        
        createContrastTexture(1024, 512, density, exponent, 
			      sm_nContrastTexture);
        sm_bContrastTextureInitialized = TRUE;
    }
    
    vertex_program = loadshaderfile(m_qsJahBasePath+"source/Jahshaka/JahLibraries/shaders/charcoal_vert_gpu.vp");

    //qDebug("PATH:"+m_qsJahBasePath);
    ...
    glProgramParameter4fNV(GL_VERTEX_PROGRAM_NV, 18, m_pCore->getCameraDistance(), 0, 0, 0);
    ...
    glBindTexture(GL_TEXTURE_2D, sm_nContrastTexture);
    
    glActiveTextureARB(GL_TEXTURE2_ARB);
    glBindTexture(GL_TEXTURE_2D, m_hRandomTexture);
    
    fragment_program = loadshaderfile(m_qsJahBasePath+"source/Jahshaka/JahLibraries/shaders/charcoal_frag_gpu.fp");
    glGenProgramsNV(1, &fragment_program_handle);
    glLoadProgramNV(GL_FRAGMENT_PROGRAM_NV, fragment_program_handle, 
        (GLsizei)strlen((char*)fragment_program), (const GLubyte*)fragment_program);
...
void GLWorld::doNV30LightingGPU(JahLayer* base_layer) {
    ...
    vertex_program = loadshaderfile(m_qsJahBasePath+"source/Jahshaka/JahLibraries/shaders/lighting_vert_gpu.vp");
    glGenProgramsNV(1, &vertex_program_handle);
    glBindProgramNV(GL_VERTEX_PROGRAM_NV, vertex_program_handle);
    glLoadProgramNV(GL_VERTEX_PROGRAM_NV, vertex_program_handle, (GLsizei)strlen((char*)vertex_program), vertex_program);
    glTrackMatrixNV(GL_VERTEX_PROGRAM_NV, 0, GL_MODELVIEW_PROJECTION_NV, GL_IDENTITY_NV);
    ...
    glProgramParameter4fNV(GL_VERTEX_PROGRAM_NV, 17, m_pCore->getCameraDistance(), 0, 0, 0);
    ...
    fragment_program = loadshaderfile(m_qsJahBasePath+"source/Jahshaka/JahLibraries/shaders/lighting_frag_gpu.fp");
    glGenProgramsNV(1, &fragment_program_handle);
    glLoadProgramNV(GL_FRAGMENT_PROGRAM_NV, fragment_program_handle, 
        (GLsizei)strlen((char*)fragment_program), (const GLubyte*)fragment_program);
...
GLWorld::saveEffectRenderingInBaseTexture(JahLayer* base_layer)
{
    ...
    int lower_left_pixel_offset_x = ( m_pCore->getRenderWidth() - width ) / 2;
    int lower_left_pixel_offset_y = ( m_pCore->getRenderHeight() - height ) / 2;
...
/*
  TODO: why is there code embedded like this?
*/
...
GLWorld::glslChromaKeyer(JahLayer* key_layer, GLuint base_texture_id, GLuint key_texture_id)
{
    ...
    if (m_pShaderProgram == NULL)
    {
        ...
        m_pFragmentShader = new GlslShader(GL_FRAGMENT_SHADER_ARB, keyer_module_fragment_shader);
        ...
        m_hSelectColorsIndividuallyHandle = 
            glGetUniformLocationARB(m_hShaderProgramHandle, "select_colors_individually");
        m_hCreateChromaAlphaMaskHandle =
            glGetUniformLocationARB(m_hShaderProgramHandle, "create_chroma_alpha_mask");
        m_hRedLimitsHandle = 
            glGetUniformLocationARB(m_hShaderProgramHandle, "red_limits");
        m_hGreenLimitsHandle = 
            glGetUniformLocationARB(m_hShaderProgramHandle, "green_limits");
        m_hBlueLimitsHandle = 
            glGetUniformLocationARB(m_hShaderProgramHandle, "blue_limits");
        m_hClearColorHandle = 
            glGetUniformLocationARB(m_hShaderProgramHandle, "clear_color");
        m_hCsoftenValueHandle = 
            glGetUniformLocationARB(m_hShaderProgramHandle, "soften_value");
    ...
    m_pShaderProgram->setTextureHandle("base_texture", 0);
    m_pShaderProgram->setTextureHandle("key_texture", 1);
    ...
    glUniform1iARB(m_hSelectColorsIndividuallyHandle, select_colors_individually);
    glUniform1iARB(m_hCreateChromaAlphaMaskHandle, create_chroma_alpha_mask);
    glUniform2fARB(m_hRedLimitsHandle, 
		   m_fRedLow - rounding_error_tolerance, 
		   m_fRedHigh + rounding_error_tolerance);
    glUniform2fARB(m_hGreenLimitsHandle, 
		   m_fGreenLow - rounding_error_tolerance, 
		   m_fGreenHigh + rounding_error_tolerance);
    glUniform2fARB(m_hBlueLimitsHandle, 
		   m_fBlueLow - rounding_error_tolerance, 
		   m_fBlueHigh + rounding_error_tolerance);
    glUniform4fARB(m_hClearColorHandle, clear_color.x, clear_color.y, clear_color.z, clear_color.w);
    glUniform1fARB(m_hCsoftenValueHandle, m_fSoftenValue);
...
void GLWorld::doChromaKeyerGLSL(JahLayer* key_layer) 
{
    ...
    glCopyTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, (m_pCore->getRenderWidth() - image_width) / 2,
        (m_pCore->getRenderHeight() - image_height) / 2, image_width, image_height);
...
const char* 
GLWorld::sm_sTrackerFragmentShader = 
{
...
GLWorld::doStabilizeGLSL(JahLayer* image_layer)
{
    ...
    if ( stabilizing_state == NULL )
    {
	  ...
	  new GlslProgram(
			  stabilizing_state->m_shader_vector
			  );
...
{
  std::cerr << "strecth crop crashes "  <<endl;
  return ;
