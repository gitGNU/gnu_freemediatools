...
bool 
GLWorld::buildLayer(JahLayer* layer, QString layerName, LayerCategory::TYPE layertype, ObjectCategory::TYPE obm_pJtype ) 
{
  ...
  if (!layer)
    ...
    switch (layertype) 
    {
        ...
        {   
            ...
            { 
                layer->asset.location  = m_qsJahMediaPath;
            }

            //this adds a 3dsmax type object
            if ( obm_pJtype==ObjectCategory::OBJECT) 
            {
...
bool 
GLWorld::loadLayer( JahLayer* jah_layer, std::string layer_name, LayerCategory::TYPE layertype, ObjectCategory::TYPE obm_pJtype, 
                   bool isasset, assetData asset, bool haskey, assetData keyasset)
{
    ...
    return loadLayer(layer_number, name_qstring, layertype, obm_pJtype, isasset, asset, haskey, keyasset);
}

bool GLWorld::loadLayer( int nl, QString layerName, LayerCategory::TYPE layertype, ObjectCategory::TYPE obm_pJtype, 
                        bool isasset, assetData asset, bool haskey, assetData keyasset ) 
{
    ...
    switch (layertype) 
    {
        ...
        {    
            ...
            if (jah_layer->lighting)
            {
                ...
                //m_aJahModuleOptions[1]->setChecked(true); never do this it break the player
        ...
        {   
            ...
            { 
                jah_layer->asset.location  = m_qsJahMediaPath;
            }

            //this adds a obj type object   
            /*
            if ( obm_pJtype==ObjectCategory::OBJOBJECTS) 
            {
...
#ifndef JAHPLAYER
                if (m_qsModuleName != "Text")
                {
                    m_aJahModuleOptions[1]->setChecked(true);
                }
