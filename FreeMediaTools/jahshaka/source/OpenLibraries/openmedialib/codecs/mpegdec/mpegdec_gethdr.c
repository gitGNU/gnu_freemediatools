...
int Get_Hdr()
{
  ...
  for (;;)
  {
    ...
    case DECODER_SEQUENCE_HEADER_CODE:
      sequence_header();
      break;
    case DECODER_GROUP_START_CODE:
      group_of_pictures_header();
      break;
    case DECODER_PICTURE_START_CODE:
      picture_header();
      return 1;
      break;
    case DECODER_SEQUENCE_END_CODE:
      return 0;
      break;
    default:
      if (!Quiet_Flag && !mpeg_data.dont_write)
...
static void sequence_header()
{
  ...
  {
    for (i=0; i<64; i++)
      ld->intra_quantizer_matrix[mpegdec_scan[DECODER_ZIG_ZAG][i]] = Get_Bits(8);
  }
  else
  {
    for (i=0; i<64; i++)
      ld->intra_quantizer_matrix[i] = mpegdec_default_intra_quantizer_matrix[i];
  }

  if((ld->load_non_intra_quantizer_matrix = Get_Bits(1)))
  {
    for (i=0; i<64; i++)
      ld->non_intra_quantizer_matrix[mpegdec_scan[DECODER_ZIG_ZAG][i]] = Get_Bits(8);
  }
  else
  {
    for (i=0; i<64; i++)
...
static void picture_header()
{
  ...
  if (picture_coding_type==DECODER_P_TYPE || picture_coding_type==DECODER_B_TYPE)
  {
...
#ifdef VERBOSE
  if (Verbose_Flag>DECODER_NO_LAYER)
  {
    printf("picture header (byte %d)\n",(pos>>3)-4);
    if (Verbose_Flag>DECODER_SEQUENCE_LAYER)
    {
...
int slice_header()
{
  ...
  if (ld->scalable_mode==DECODER_SC_DP)
...
static void extension_and_user_data()
{
  ...
  while ((code = Show_Bits(32))==DECODER_EXTENSION_START_CODE || code==DECODER_USER_DATA_START_CODE)
  {
    if (code==DECODER_EXTENSION_START_CODE)
    {
      Flush_Buffer32();
      ext_ID = Get_Bits(4);
      switch (ext_ID)
      {
      case DECODER_SEQUENCE_EXTENSION_ID:
        sequence_extension();
        break;
      case DECODER_SEQUENCE_DISPLAY_EXTENSION_ID:
        sequence_display_extension();
        break;
      case DECODER_QUANT_MATRIX_EXTENSION_ID:
        quant_matrix_extension();
        break;
      case DECODER_SEQUENCE_SCALABLE_EXTENSION_ID:
        sequence_scalable_extension();
        break;
      case DECODER_PICTURE_DISPLAY_EXTENSION_ID:
        picture_display_extension();
        break;
      case DECODER_PICTURE_CODING_EXTENSION_ID:
        picture_coding_extension();
        break;
      case DECODER_PICTURE_SPATIAL_SCALABLE_EXTENSION_ID:
        picture_spatial_scalable_extension();
        break;
      case DECODER_PICTURE_TEMPORAL_SCALABLE_EXTENSION_ID:
        picture_temporal_scalable_extension();
        break;
      case DECODER_COPYRIGHT_EXTENSION_ID:
        copyright_extension();
        break;
     default:
...
static void sequence_extension()
{
  ...
  ld->scalable_mode = DECODER_SC_NONE; /* unless overwritten by sequence_scalable_extension() */
  ...
  {  /* escape bit of profile_and_level_indication set */
    ...
    {
      profile = DECODER_PROFILE_422;
      level   = DECODER_MAIN_LEVEL;  
    }
  }
  else
  {
...
static void quant_matrix_extension()
{
  ...
  if((ld->load_intra_quantizer_matrix = Get_Bits(1)))
  {
    for (i=0; i<64; i++)
    {
      ld->chroma_intra_quantizer_matrix[mpegdec_scan[DECODER_ZIG_ZAG][i]]
      = ld->intra_quantizer_matrix[mpegdec_scan[DECODER_ZIG_ZAG][i]]
      = Get_Bits(8);
    }
  }
  ...
  if((ld->load_non_intra_quantizer_matrix = Get_Bits(1)))
  {
    for (i=0; i<64; i++)
    {
      ld->chroma_non_intra_quantizer_matrix[mpegdec_scan[DECODER_ZIG_ZAG][i]]
      = ld->non_intra_quantizer_matrix[mpegdec_scan[DECODER_ZIG_ZAG][i]]
      = Get_Bits(8);
    }
  }
  ...
  {
    for (i=0; i<64; i++)
      ld->chroma_intra_quantizer_matrix[mpegdec_scan[DECODER_ZIG_ZAG][i]] = Get_Bits(8);
  }

  if((ld->load_chroma_non_intra_quantizer_matrix = Get_Bits(1)))
  {
    for (i=0; i<64; i++)
      ld->chroma_non_intra_quantizer_matrix[mpegdec_scan[DECODER_ZIG_ZAG][i]] = Get_Bits(8);
  }
...
static void sequence_scalable_extension()
{
  ...
  if (ld->scalable_mode==DECODER_SC_SPAT)
  {
  ...
  if (ld->scalable_mode==DECODER_SC_TEMP)
...
static void Update_Temporal_Reference_Tacking_Data()
{
  ...
  if (ld == &base)			/* *CH* */
  {
    if (picture_coding_type!=DECODER_B_TYPE && temporal_reference!=temporal_reference_old) 	
