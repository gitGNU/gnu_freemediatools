...
int dc_dct_pred[];
{
  ...
  for (i=1; ; i++)
  {
    ...
    j = mpegdec_scan[DECODER_ZIG_ZAG][i];
...
int dc_dct_pred[];
{
  ...
  ld1 = (ld->scalable_mode==DECODER_SC_DP) ? &base : ld;
  bp = ld1->block[comp];
  ...
  qmat = (comp<4 || chroma_format==DECODER_CHROMA420)
  ...
  for (i=1; ; i++)
  {
    ...
    j = mpegdec_scan[ld1->alternate_scan][i];
    val = (val * ld1->quantizer_scale * qmat[j]) >> 4;
    bp[j] = sign ? -val : val;
    nc++;

    if (base.scalable_mode==DECODER_SC_DP && nc==base.priority_breakpoint-63)
      ld = &enhan;
  }
}
