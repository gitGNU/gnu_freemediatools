...
{
  int hsize;
  char tmpname[DECODER_FILENAME_LENGTH];
...
int tgaflag;
{
  ...
  else
  {
      if (!u444)
      {
          if (chroma_format==DECODER_CHROMA420)
          {
              if (!(u422 = (unsigned char *)malloc(MAX_WIDTH * MAX_HEIGHT) ) )
              {
                  Error("malloc failed");
              }
              if (!(v422 = (unsigned char *)malloc(MAX_WIDTH * MAX_HEIGHT) ) )
              {
                  Error("malloc failed");
              }
          }

          if (!(u444 = (unsigned char *)malloc(MAX_WIDTH * MAX_HEIGHT) ) )
          {
              Error("malloc failed");
          }

          if (!(v444 = (unsigned char *)malloc(MAX_WIDTH * MAX_HEIGHT) ) )
          {
              Error("malloc failed");
          }
      }
...
*/
  ...
  // This line keeps Visual Studio from complaining about the label
  current_pixel = 0;
  //close(outfile);
