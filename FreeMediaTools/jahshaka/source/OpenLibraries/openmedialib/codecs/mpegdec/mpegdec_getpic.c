...
int bitstream_framenum, sequence_framenum;
{
  ...
  if (picture_structure==DECODER_FRAME_PICTURE && Second_Field)
  {
  ...
  if (picture_structure!=DECODER_FRAME_PICTURE)
...
int framenum, MBAmax;
{
  ...
  if (Two_Streams && enhan.scalable_mode==DECODER_SC_SNR)
  {
    SNRMBA=0;
    SNRMBAinc=0;
  }
  ...
  for (;;)
  {
    ...
    if (MBAinc==0)
    {
      if (base.scalable_mode==DECODER_SC_DP && base.priority_breakpoint==1)
          ld = &enhan;
  ...
  int *pdct_type;
{
  ...
  if (macroblock_type & DECODER_MB_WEIGHT)
  {
    ...
    stwtype = (macroblock_type & DECODER_MB_CLASS4) ? 8 : 0;

  /* SCALABILITY: derive spatial_temporal_weight_class (Table 7-18) */
  stwclass = stwclass_table[stwtype];

  /* get frame/field motion type */
  if (macroblock_type & (DECODER_MACROBLOCK_MOTION_FORWARD|DECODER_MACROBLOCK_MOTION_BACKWARD))
  {
    if (picture_structure==DECODER_FRAME_PICTURE) /* frame_motion_type */
    {
      motion_type = frame_pred_frame_dct ? DECODER_MC_FRAME : Get_Bits(2);
#ifdef TRACE
  ...
  else if ((macroblock_type & DECODER_MACROBLOCK_INTRA) && concealment_motion_vectors)
  {
    /* concealment motion vectors */
    motion_type = (picture_structure==DECODER_FRAME_PICTURE) ? DECODER_MC_FRAME : DECODER_MC_FIELD;
  }
#if 0
  else
  {
  ...
  if (picture_structure==DECODER_FRAME_PICTURE)
  {
    motion_vector_count = (motion_type==DECODER_MC_FIELD && stwclass<2) ? 2 : 1;
    mv_format = (motion_type==DECODER_MC_FRAME) ? DECODER_MV_FRAME : DECODER_MV_FIELD;
  }
  else
  {
    motion_vector_count = (motion_type==DECODER_MC_16X8) ? 2 : 1;
    mv_format = DECODER_MV_FIELD;
  }

  dmv = (motion_type==DECODER_MC_DMV); /* dual prime */
  ...
  mvscale = ((mv_format==DECODER_MV_FIELD) && (picture_structure==DECODER_FRAME_PICTURE));

  /* get dct_type (frame DCT / field DCT) */
  dct_type = (picture_structure==DECODER_FRAME_PICTURE)
             && (!frame_pred_frame_dct)
             && (macroblock_type & (DECODER_MACROBLOCK_PATTERN|DECODER_MACROBLOCK_INTRA))
             ? Get_Bits(1)
             : 0;

#ifdef TRACE
  if (Trace_Flag  && (picture_structure==DECODER_FRAME_PICTURE)
             && (!frame_pred_frame_dct)
...
int comp,bx,by,dct_type,addflag;
{
  ...
  else
  {
    ...
    /* scale coordinates */
    if (chroma_format!=DECODER_CHROMA444)
      bx >>= 1;
    if (chroma_format==DECODER_CHROMA420)
      by >>= 1;
    if (picture_structure==DECODER_FRAME_PICTURE)
    {
      if (dct_type && (chroma_format!=DECODER_CHROMA420))
      {
        /* field DCT coding */
  ...
  int *dct_type;
{
  ...
  if (*SNRMBAinc==0)
  {
    if (!Show_Bits(23)) /* next_start_code */
    {
      ...
      if (code<DECODER_SLICE_START_CODE_MIN || code>DECODER_SLICE_START_CODE_MAX)
      {
  ...
  if (*SNRMBAinc==1) /* not skipped */
  {
    ...
    if (SNRmacroblock_type & DECODER_MACROBLOCK_PATTERN)
      *dct_type = SNRdct_type;

    if (SNRmacroblock_type & DECODER_MACROBLOCK_QUANT)
    {
...
{                           
  ...
  for (cc=0; cc<3; cc++)
  {
    ...
    if (picture_coding_type==DECODER_B_TYPE)
    {
    ...
    if (picture_structure==DECODER_BOTTOM_FIELD)
...
int dct_type;
{
  ...
  for (comp=0; comp<block_count; comp++)
  {
    ...
    /* ISO/IEC 13818-2 section 7.4.3 and 7.4.4: Saturation and Mismatch control */
    if ((Two_Streams && enhan.scalable_mode==DECODER_SC_SNR) || ld->MPEG2_Flag)
    ...
    /* ISO/IEC 13818-2 section 7.6.8: Adding prediction and coefficient data */
    Add_Block(comp,bx,by,dct_type,(macroblock_type & DECODER_MACROBLOCK_INTRA)==0);
  }
...
int *macroblock_type;
{
  ...
  if (picture_coding_type==DECODER_P_TYPE)
    PMV[0][0][0]=PMV[0][0][1]=PMV[1][0][0]=PMV[1][0][1]=0;

  /* derive motion_type */
  if (picture_structure==DECODER_FRAME_PICTURE)
    *motion_type = DECODER_MC_FRAME;
  else
  {
    *motion_type = DECODER_MC_FIELD;

    /* predict from field of same parity */
    /* ISO/IEC 13818-2 section 7.6.6.1 and 7.6.6.3: P field picture and B field
       picture */
    motion_vertical_field_select[0][0]=motion_vertical_field_select[0][1] = 
      (picture_structure==DECODER_BOTTOM_FIELD);
  }

  /* skipped I are spatial-only predicted, */
  /* skipped P and B are temporal-only predicted */
  /* ISO/IEC 13818-2 section 7.7.6: Skipped macroblocks */
  *stwtype = (picture_coding_type==DECODER_I_TYPE) ? 8 : 0;

 /* IMPLEMENTATION: clear MACROBLOCK_INTRA */
  *macroblock_type&= ~DECODER_MACROBLOCK_INTRA;
...
int dmvector[2];
{
  ...
  if (*macroblock_type & DECODER_MACROBLOCK_QUANT)
  {
    quantizer_scale_code = Get_Bits(5);
  ...
  if ((*macroblock_type & DECODER_MACROBLOCK_MOTION_FORWARD) 
    || ((*macroblock_type & DECODER_MACROBLOCK_INTRA) 
  ...
  if (*macroblock_type & DECODER_MACROBLOCK_MOTION_BACKWARD)
  {
    if (ld->MPEG2_Flag)
      motion_vectors(PMV,dmvector,motion_vertical_field_select,
  ...
  if ((*macroblock_type & DECODER_MACROBLOCK_INTRA) && concealment_motion_vectors)
    Flush_Buffer(1); /* remove marker_bit */

  if (base.scalable_mode==DECODER_SC_DP && base.priority_breakpoint==3)
    ld = &enhan;

  /* macroblock_pattern */
  /* ISO/IEC 13818-2 section 6.3.17.4: Coded block pattern */
  if (*macroblock_type & DECODER_MACROBLOCK_PATTERN)
  {
    ...
    coded_block_pattern = (*macroblock_type & DECODER_MACROBLOCK_INTRA) ? 
      (1<<block_count)-1 : 0;
  ...
  if(picture_coding_type==DECODER_D_TYPE)
  {
  ...
  if ((picture_coding_type==DECODER_P_TYPE) 
    && !(*macroblock_type & (DECODER_MACROBLOCK_MOTION_FORWARD|DECODER_MACROBLOCK_INTRA)))
  {
    ...
    else
    {
      *motion_type = DECODER_MC_FIELD;
      /* predict from field of same parity */
      motion_vertical_field_select[0][0] = (picture_structure==DECODER_BOTTOM_FIELD);
    }
  }
