...
int sequence_framenum;
{
    ...
    if((sequence_framenum!=0)||(Second_Field))
    {
        ...
        if ((picture_structure==DECODER_FRAME_PICTURE)||(!Second_Field))
        {
            ...
            else if((picture_coding_type==DECODER_B_TYPE)&&(bgate!=1))
            {
                substitute_display_framenum = 
        ...
        else if(Second_Field && (picture_coding_type==DECODER_P_TYPE))
        {
            /* our favourite case: the IP field picture pair */
            if((previous_picture_coding_type==DECODER_I_TYPE)&&(picture_coding_type==DECODER_P_TYPE))
            {
    ...
    if((picture_coding_type!=DECODER_B_TYPE) && 
        ((picture_structure==DECODER_FRAME_PICTURE)||Second_Field))
    {
...
int framenum;
{
    ...
    if((Second_Field)&&(picture_coding_type==DECODER_P_TYPE))
    {
        parity      = (picture_structure==DECODER_TOP_FIELD ? 1:0);      
        field_mode  = (picture_structure==DECODER_FRAME_PICTURE ? 0:1);
    }
    else
    {
...
{
    int err = 0;
    char outname[DECODER_FILENAME_LENGTH];
    char name[DECODER_FILENAME_LENGTH];
...
int framenum;
{
    ...
    if(chroma_format==DECODER_CHROMA444)
        size = (size * 3);
    else if(chroma_format==DECODER_CHROMA422)
        size = (size * 2);
    else if(chroma_format==DECODER_CHROMA420)
        size = ((size*3)>>1);
    else
