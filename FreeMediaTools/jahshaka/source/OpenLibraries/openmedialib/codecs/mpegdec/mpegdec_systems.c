...
void Next_Packet()
{
    ...
    for(;;)
    {
        ...
        switch(code)
        {
        case DECODER_PACK_START_CODE: /* pack header */
        ...
        case DECODER_VIDEO_ELEMENTARY_STREAM:   
            ...
            return;
        case DECODER_ISO_END_CODE: /* end */
            /* simulate a buffer full of sequence end codes */
            l = 0;
            while (l<2048)
            {
                ld->Rdbfr[l++] = DECODER_SEQUENCE_END_CODE>>24;
                ld->Rdbfr[l++] = DECODER_SEQUENCE_END_CODE>>16;
                ld->Rdbfr[l++] = DECODER_SEQUENCE_END_CODE>>8;
                ld->Rdbfr[l++] = DECODER_SEQUENCE_END_CODE&0xff;
            }
            ld->Rdptr = ld->Rdbfr;
            ld->Rdmax = ld->Rdbfr + 2048;
            return;
        default:
            if(code>=DECODER_SYSTEM_START_CODE)
            {
