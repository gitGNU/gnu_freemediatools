...
int stwtype;
{
    ...
    if ((macroblock_type & DECODER_MACROBLOCK_MOTION_FORWARD) 
        || (picture_coding_type==DECODER_P_TYPE))
    {
        if (picture_structure==DECODER_FRAME_PICTURE)
        {
            if ((motion_type==DECODER_MC_FRAME) 
                || !(macroblock_type & DECODER_MACROBLOCK_MOTION_FORWARD))
            {
            ...
            else if (motion_type==DECODER_MC_FIELD) /* field-based prediction */
            {
            ...
            else if (motion_type==DECODER_MC_DMV) /* dual prime prediction */
            {
        ...
        {
            /* field picture */
            currentfield = (picture_structure==DECODER_BOTTOM_FIELD);
            
            /* determine which frame to use for prediction */
            if ((picture_coding_type==DECODER_P_TYPE) && Second_Field
            ...
            if ((motion_type==DECODER_MC_FIELD)
                || !(macroblock_type & DECODER_MACROBLOCK_MOTION_FORWARD))
            {
            ...
            else if (motion_type==DECODER_MC_16X8)
            {
                if (stwtop<2)
                {
                    form_prediction(predframe,motion_vertical_field_select[0][0],current_frame,0,
