	...
	struct mbinfo* mbi)
{
	...
	for (j = 0; j < m_height2; j += 16)
    {
		...
		{
			predict_mb(reff, refb, cur, m_width, i, j, m_pict_type, m_pict_struct,
...
void 
MpegEncoder::calc_DMV(int DMV[][2], int* dmvector, int mvx, int mvy)
{
...
void 
MpegEncoder::clearblock(unsigned char* cur[], int i0, int j0)
{
	int i, j, w, h;
	unsigned char *p;

	p = cur[0] + ((m_pict_struct == BOTTOM_FIELD) ? m_width : 0) + i0 + (m_width2 * j0);
	...
	p = cur[1] + ((m_pict_struct == BOTTOM_FIELD) ? m_chrom_width : 0) + i0
		+ (m_chrom_width2 * j0);
	...
	p = cur[2] + ((m_pict_struct == BOTTOM_FIELD) ? m_chrom_width : 0) + i0
		+ m_chrom_width2*j0;
