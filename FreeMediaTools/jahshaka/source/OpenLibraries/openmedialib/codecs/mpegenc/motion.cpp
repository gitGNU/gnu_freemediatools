...
struct mbinfo* mbi)
{
    ...
    var = variance(mb, m_width);

    if (m_pict_type == I_TYPE)
    {
        mbi->mb_type = MB_INTRA;
    }
    else if (m_pict_type == P_TYPE)
    {
        if (m_frame_pred_dct)
        {
            dmc = fullsearch(oldorg,oldref,mb,
                m_width, i, j, sxf, syf, 16, m_width, m_height, &imin, &jmin);
            vmc = dist2(oldref+(imin>>1)+ m_width * (jmin>>1),mb,
                m_width, imin&1, jmin&1,16);
        ...
        else
        {
            ...
            if (m_M == 1 && dmc_dp < dmc && dmc_dp < dmcfield)
            {
            ...
            else
            {
                ...
                dmc = dmcfield;
                vmc = dist2(oldref +(tsel? m_width : 0)+(imint>>1) + (m_width << 1) * (jmint>>1),
                    mb, m_width << 1, imint&1, jmint&1, 8);
                vmc += dist2(oldref + (bsel? m_width:0)+(iminb>>1) + (m_width << 1) * (jminb>>1),
                    mb + m_width, m_width << 1, iminb & 1, jminb & 1, 8);
            }
        }
        ...
        */
        ...
        else
        {
            ...
            */
            v0 = dist2(oldref + i + m_width * j, mb, m_width,0,0,16);
            if (4*v0>5*vmc && v0>=9*256)
            {
                ...
                {
                    /* these are LayerCategory::FRAME vectors */
                    /* same parity vector */
    ...
    else /* if (pict_type==B_TYPE) */
    {
        if (m_frame_pred_dct)
        {
            ...
            dmcf = fullsearch(oldorg,oldref,mb,
                m_width, i, j, sxf, syf,16, m_width, m_height,&iminf,&jminf);
            vmcf = dist2(oldref+(iminf>>1) + m_width * (jminf>>1),mb,
                m_width, iminf & 1, jminf & 1, 16);

            /* backward */
            dmcr = fullsearch(neworg,newref,mb,
                m_width, i, j, sxb, syb, 16, m_width, m_height, &iminr, &jminr);
            vmcr = dist2(newref+(iminr>>1) + ( m_width * (jminr >> 1) ) ,mb,
                m_width, iminr & 1, jminr & 1, 16);

            /* interpolated (bidirectional) */
            vmci = bdist2(oldref+(iminf>>1) + m_width * (jminf >> 1),
                newref + (iminr>>1) + m_width * (jminr>>1),
                mb, m_width, iminf & 1, jminf & 1, iminr & 1, jminr & 1, 16);
        ...
        else
        {
            ...
            /* frame */
            dmci = bdist1(oldref+(iminf>>1)+ m_width * (jminf>>1),
                newref + (iminr>>1) + m_width * (jminr>>1),
                mb, m_width, iminf&1, jminf&1, iminr&1, jminr&1,16);

            /* top field */
            dmcfieldi = bdist1(
                oldref + (imintf>>1) + (tself ? m_width:0) + (m_width<<1)*(jmintf>>1),
                newref+(imintr>>1)+(tselr ? m_width:0) + (m_width<<1)*(jmintr>>1),
                mb, m_width<<1,imintf&1,jmintf&1,imintr&1,jmintr&1,8);

            /* bottom field */
            dmcfieldi+= bdist1(
                oldref+(iminbf>>1)+(bself? m_width : 0)+( m_width << 1) * ( jminbf >> 1),
                newref+(iminbr>>1)+(bselr ? m_width : 0)+(m_width << 1) * ( jminbr >> 1),
                mb + m_width, m_width << 1, iminbf & 1, jminbf & 1,iminbr & 1,jminbr & 1, 8);
            ...
            */
                ...
                && dmci<dmcr && dmci<dmcfieldr)
            {
                ...
                mbi->mb_type = MB_FORWARD|MB_BACKWARD;
                mbi->motion_type = MC_FRAME;
                vmc = bdist2(oldref+(iminf>>1) + m_width * (jminf>>1),
                    newref + (iminr>>1) + m_width * (jminr>>1),
                    mb,m_width,iminf&1,jminf&1,iminr&1,jminr&1,16);
            }
                ...
                && dmcfieldi<dmcr && dmcfieldi<dmcfieldr)
            {
                ...
                mbi->mb_type = MB_FORWARD|MB_BACKWARD;
                mbi->motion_type = MC_FIELD;
                vmc = bdist2(oldref+(imintf>>1)+(tself?m_width:0)+(m_width<<1)*(jmintf>>1),
                    newref+(imintr>>1)+(tselr?m_width:0)+(m_width<<1)*(jmintr>>1),
                    mb,m_width<<1,imintf&1,jmintf&1,imintr&1,jmintr&1,8);
                vmc+= bdist2(oldref+(iminbf>>1)+(bself?m_width:0)+(m_width<<1)*(jminbf>>1),
                    newref+(iminbr>>1)+(bselr?m_width:0)+(m_width<<1)*(jminbr>>1),
                    mb+m_width,m_width<<1,iminbf&1,jminbf&1,iminbr&1,jminbr&1,8);
            }
            else if (dmcf<dmcfieldf && dmcf<dmcr && dmcf<dmcfieldr)
            {
                ...
                mbi->mb_type = MB_FORWARD;
                mbi->motion_type = MC_FRAME;
                vmc = dist2(oldref+(iminf>>1)+m_width*(jminf>>1),mb,
                    m_width,iminf&1,jminf&1,16);
            }
            else if (dmcfieldf<dmcr && dmcfieldf<dmcfieldr)
            {
                ...
                mbi->mb_type = MB_FORWARD;
                mbi->motion_type = MC_FIELD;
                vmc = dist2(oldref+(tself?m_width:0)+(imintf>>1)+(m_width<<1)*(jmintf>>1),
                    mb,m_width<<1,imintf&1,jmintf&1,8);
                vmc+= dist2(oldref+(bself?m_width:0)+(iminbf>>1)+(m_width<<1)*(jminbf>>1),
                    mb+m_width,m_width<<1,iminbf&1,jminbf&1,8);
            }
            else if (dmcr<dmcfieldr)
            {
                ...
                mbi->mb_type = MB_BACKWARD;
                mbi->motion_type = MC_FRAME;
                vmc = dist2(newref+(iminr>>1)+m_width*(jminr>>1),mb,
                    m_width,iminr&1,jminr&1,16);
            }
            else
            {
                ...
                mbi->mb_type = MB_BACKWARD;
                mbi->motion_type = MC_FIELD;
                vmc = dist2(newref+(tselr?m_width:0)+(imintr>>1)+(m_width<<1)*(jmintr>>1),
                    mb,m_width<<1,imintr&1,jmintr&1,8);
                vmc+= dist2(newref+(bselr?m_width:0)+(iminbr>>1)+(m_width<<1)*(jminbr>>1),
                    mb+m_width,m_width<<1,iminbr&1,jminbr&1,8);
            }
        }
    ...
    int ipflag)
{
    ...
    else if (m_pict_type == P_TYPE)
    {
        toporg = oldorg;
        topref = oldref;
        botorg = oldorg + m_width;
        botref = oldref + m_width;

        if (secondfield)
        {
            /* opposite parity field is in same frame */
            if (m_pict_struct == TOP_FIELD)
            {
                /* current is top field */
                botorg = cur + m_width;
                botref = curref + m_width;
            }
            else
            {
        ...
        else
        {
            ...
            */
            if (!ipflag)
            {
                v0 = dist2(((m_pict_struct == BOTTOM_FIELD)? botref : topref) + i + w2*j,
                    mb,w2,0,0,16);
            }
            ...
            else
            {
                ...
                mbi->MV[0][0][0] = 0;
                mbi->MV[0][0][1] = 0;
                mbi->mv_field_sel[0][0] = (m_pict_struct == BOTTOM_FIELD);
            }
        }
    }
    else /* if (pict_type==B_TYPE) */
    {
        /* forward prediction */
        field_estimate(oldorg,oldref,oldorg+m_width,oldref+m_width,mb,
            i,j,sxf,syf,0,
            &iminf,&jminf,&imin8uf,&jmin8uf,&imin8lf,&jmin8lf,
            &dmcfieldf,&dmc8f,&self,&sel8uf,&sel8lf,&imins,&jmins,&ds);

        /* backward prediction */
        field_estimate(neworg,newref,neworg+m_width,newref+m_width,mb,
            i,j,sxb,syb,0,
        ...
        /* field */
        dmcfieldi = bdist1(oldref + (self?m_width:0) + (iminf>>1) + w2*(jminf>>1),
            newref + (selr?m_width:0) + (iminr>>1) + w2*(jminr>>1),
            mb,w2,iminf&1,jminf&1,iminr&1,jminr&1,16);

        /* 16x8 upper half block */
        dmc8i = bdist1(oldref + (sel8uf?m_width:0) + (imin8uf>>1) + w2*(jmin8uf>>1),
            newref + (sel8ur?m_width:0) + (imin8ur>>1) + w2*(jmin8ur>>1),
            mb,w2,imin8uf&1,jmin8uf&1,imin8ur&1,jmin8ur&1,8);

        /* 16x8 lower half block */
        dmc8i+= bdist1(oldref + (sel8lf?m_width:0) + (imin8lf>>1) + w2*(jmin8lf>>1),
            newref + (sel8lr?m_width:0) + (imin8lr>>1) + w2*(jmin8lr>>1),
            ...
            && dmcfieldi<dmcfieldr && dmcfieldi<dmc8r)
        {
            ...
            mbi->mb_type = MB_FORWARD|MB_BACKWARD;
            mbi->motion_type = MC_FIELD;
            vmc = bdist2(oldref + (self?m_width:0) + (iminf>>1) + w2*(jminf>>1),
                newref + (selr?m_width:0) + (iminr>>1) + w2*(jminr>>1),
                mb,w2,iminf&1,jminf&1,iminr&1,jminr&1,16);
        }
            ...
            && dmc8i<dmcfieldr && dmc8i<dmc8r)
        {
            ...
            /* upper half block */
            vmc = bdist2(oldref + (sel8uf?m_width:0) + (imin8uf>>1) + w2*(jmin8uf>>1),
                newref + (sel8ur?m_width:0) + (imin8ur>>1) + w2*(jmin8ur>>1),
                mb,w2,imin8uf&1,jmin8uf&1,imin8ur&1,jmin8ur&1,8);

            /* lower half block */
            vmc+= bdist2(oldref + (sel8lf?m_width:0) + (imin8lf>>1) + w2*(jmin8lf>>1),
                newref + (sel8lr?m_width:0) + (imin8lr>>1) + w2*(jmin8lr>>1),
                mb+8*w2,w2,imin8lf&1,jmin8lf&1,imin8lr&1,jmin8lr&1,8);
        }
        else if (dmcfieldf<dmc8f && dmcfieldf<dmcfieldr && dmcfieldf<dmc8r)
        {
            ...
            mbi->mb_type = MB_FORWARD;
            mbi->motion_type = MC_FIELD;
            vmc = dist2(oldref + (self?m_width:0) + (iminf>>1) + w2*(jminf>>1),
        ...
        else if (dmc8f<dmcfieldr && dmc8f<dmc8r)
        {
            ...
            /* upper half block */
            vmc = dist2(oldref + (sel8uf?m_width:0) + (imin8uf>>1) + w2*(jmin8uf>>1),
                mb,w2,imin8uf&1,jmin8uf&1,8);

            /* lower half block */
            vmc+= dist2(oldref + (sel8lf?m_width:0) + (imin8lf>>1) + w2*(jmin8lf>>1),
        ...
        else if (dmcfieldr<dmc8r)
        {
            ...
            mbi->mb_type = MB_BACKWARD;
            mbi->motion_type = MC_FIELD;
            vmc = dist2(newref + (selr?m_width:0) + (iminr>>1) + w2*(jminr>>1),
        ...
        else
        {
            ...
            /* upper half block */
            vmc = dist2(newref + (sel8ur?m_width:0) + (imin8ur>>1) + w2*(jmin8ur>>1),
                mb,w2,imin8ur&1,jmin8ur&1,8);

            /* lower half block */
            vmc+= dist2(newref + (sel8lr?m_width:0) + (imin8lr>>1) + w2*(jmin8lr>>1),
                        ...
                        int jmins[2][2])
{
    ...
    /* frame prediction */
    *dframep = fullsearch(org, ref, mb, m_width, i, j, sx, sy, 16, m_width, m_height,
        iminp, jminp);

    /* predict top field from top field */
    dt = fullsearch(org, ref, mb, m_width << 1, i, j >> 1, sx, sy>>1, 8, 
        m_width, m_height >> 1, &imint, &jmint);

    /* predict top field from bottom field */
    db = fullsearch(org + m_width, ref + m_width, mb, m_width << 1, 
        i, j >> 1, sx, sy >> 1, 8, m_width, m_height >> 1, &iminb, &jminb);
    ...
    dt = fullsearch(org, ref, mb + m_width, m_width << 1, 
        i, j >> 1, sx, sy >> 1, 8, m_width, m_height >>1, &imint,&jmint);

    /* predict bottom field from bottom field */
    db = fullsearch(org + m_width, ref + m_width, mb + m_width, m_width << 1,
        i, j >> 1, sx, sy >> 1, 8, m_width, m_height >> 1,&iminb, &jminb);
                        ...
                        int* dsp)
{
    ...
    notop = ipflag && (m_pict_struct == TOP_FIELD);
    nobot = ipflag && (m_pict_struct == BOTTOM_FIELD);
    ...
    {
        dt = fullsearch(toporg, topref, mb, m_width << 1,
            i, j, sx, sy >> 1, 16, m_width, m_height >> 1,&imint, &jmint);
    }
    ...
    {
        db = fullsearch(botorg, botref, mb, m_width << 1, i, j, sx,
            sy >> 1, 16, m_width, m_height >> 1, &iminb, &jminb);
    }
    ...
    {
        dt = fullsearch(toporg, topref, mb, m_width << 1,
            i, j, sx, sy >> 1, 8, m_width, m_height >> 1, &imint, &jmint);
    }
    ...
    {
        db = fullsearch(botorg, botref, mb, m_width << 1,
            i, j, sx, sy >> 1, 8, m_width, m_height>>1, &iminb, &jminb);
    }
    ...
    if (notop)
        ...
        dt = fullsearch(toporg, topref, mb + (m_width << 4), m_width << 1,
        i, j + 8, sx, sy >> 1, 8, m_width, m_height >> 1, &imint, &jmint);
    ...
    {
        db = fullsearch(botorg, botref, mb + (m_width << 4), m_width << 1,
            i, j + 8, sx, sy >> 1, 8, m_width, m_height >> 1, &iminb, &jminb);
    }
                          ...
                          int* vmcp)
{
    ...
    */
                ...
                js >= 0 && js <= ((m_height-16)) )
            {
                for (delta_y = -1; delta_y <= 1; delta_y++)
                {
                    for (delta_x = -1; delta_x <= 1; delta_x++)
                    {
                        ...
                        if (it >= 0 && it <= ((m_width - 16) << 1) &&
                            jt >= 0 && jt <= ((m_height - 16)) &&
                            ib >= 0 && ib <= ((m_width - 16) << 1) &&
                            jb >= 0 && jb <= (m_height - 16) )
                        {
                            /* compute prediction error */
                            local_dist = bdist2(
                                ref + (is>>1) + (m_width<<1)*(js>>1),
                                ref + m_width + (it>>1) + (m_width<<1)*(jt>>1),
                                mb,             /* current mb location */
                                m_width<<1,       /* adjacent line distance */
                                is&1, js&1, it&1, jt&1, /* half-pel flags */
                                8);             /* block height */
                            local_dist += bdist2(
                                ref + m_width + (is>>1) + (m_width<<1)*(js>>1),
                                ref + (ib>>1) + (m_width<<1)*(jb>>1),
                                mb + m_width,     /* current mb location */
                                m_width<<1,       /* adjacent line distance */
    ...
    local_dist = bdist1(
        ref + (imins>>1) + (m_width<<1)*(jmins>>1),
        ref + m_width + (imint>>1) + (m_width<<1)*(jmint>>1),
        mb,
        m_width<<1,
        imins&1, jmins&1, imint&1, jmint&1,
        8);
    local_dist += bdist1(
        ref + m_width + (imins>>1) + (m_width<<1)*(jmins>>1),
        ref + (iminb>>1) + (m_width<<1)*(jminb>>1),
        mb + m_width,
        m_width<<1,
                          ...
                          int* vmcp)
{
    ...
    */
    ...
    for (delta_y = -1; delta_y <= 1; delta_y++)
    {
        for (delta_x = -1; delta_x <=1; delta_x++)
        {
            ...
            if (io >= 0 && io <= (m_width-16)<<1 &&
                jo >= 0 && jo <= (m_height2-16)<<1)
            {
                /* compute prediction error */
                local_dist = bdist2(
                    sameref + (imins>>1) + m_width2*(jmins>>1),
                    oppref  + (io>>1)    + m_width2*(jo>>1),
                    mb,             /* current mb location */
                    m_width2,         /* adjacent line distance */
    ...
    *dmcp = bdist1(
        sameref + (imins>>1) + m_width2*(jmins>>1),
        oppref  + (imino>>1) + m_width2*(jmino>>1),
        mb,             /* current mb location */
        m_width2,         /* adjacent line distance */
...
int 
MpegEncoder::variance(unsigned char* p, int lx)
{
    int i,j;
    unsigned int v,s,s2;
