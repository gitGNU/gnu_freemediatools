...
void 
MpegEncoder::putintrablk(short* blk, int cc)
{
	int n, dct_diff, run, signed_level;

	/* DC coefficient (7.2.1) */
	dct_diff = blk[0] - m_dc_dct_pred[cc]; /* difference to previous block */
	m_dc_dct_pred[cc] = blk[0];
	...
	for (n=1; n<64; n++)
	{
		...
		signed_level = blk[(m_altscan ? m_alternate_scan : m_zig_zag_scan)[n]];

		if (signed_level!=0)
		{
			putAC(run, signed_level, m_intravlc);
			run = 0;
		}
		else
        {
...
void 
MpegEncoder::putnonintrablk(short* blk)
{
...
void 
MpegEncoder::putmv(int dmv, int f_code)
{
