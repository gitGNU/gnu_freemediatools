	...
	case PDPlane:
		{
			...
			for(int i = 0; i < group->p_count; i++)
			{
				...
				float epsilon = 1.0f;
				pVector tmp = ((position.p2 - Vn) * (scaleFac / (dist*dist+epsilon))) + Vn;
				tmp.normalize();
				m.vel = tmp * vm;

				//m.vel += position.p2 * (scaleFac / (dist*dist));
			}
		}
		break;
...
void PACallActionList::Execute(ParticleGroup *)
{
...
void PAExplosion::Execute(ParticleGroup *group)
{
	...
	for(int i = 0; i < group->p_count; i++)
	{
		...
		m.vel += dir * (Gd * mdt / (dist * (distSqr + epsilon)));
	}
...
void PAFollow::Execute(ParticleGroup *group)
{
	...
	for(int i = 0; i < group->p_count - 1; i++)
	{
		...
		m.vel += tohim * (mdt / (sqrtf(tohimlenSqr) * (tohimlenSqr + epsilon)));
	}
}
...
void PAGravitate::Execute(ParticleGroup *group)
{
	...
	for(int i = 0; i < group->p_count; i++)
	{
		...
		for(int j = i + 1; j < group->p_count; j++)
		{
			...
			// Compute force exerted between the two bodies
			pVector acc(tohim * (mdt / (sqrtf(tohimlenSqr) * (tohimlenSqr + epsilon))));
...
void PAMatchVelocity::Execute(ParticleGroup *group)
{
	...
	for(int i = 0; i < group->p_count; i++)
	{
		...
		for(int j = i + 1; j < group->p_count; j++)
		{
			...
			// Compute force exerted between the two bodies
			pVector acc(mj.vel * (mdt / (tohimlenSqr + epsilon)));
...
void PAOrbitLine::Execute(ParticleGroup *group)
{
	...
	for(int i = 0; i < group->p_count; i++)
	{
		...
		if(rSqr < maxRadiusSqr) // XXX Removed because it causes pipeline stalls.
			// Step velocity with acceleration
